...
- **CI workflows**: kept `test.yml`, `test-vscode.yml`, `visual-regression.yml` (vscode job only, baselines in-repo), `smoke-test.yml`, `typecheck.yml`, `beta.yml`, `check-md-table-padding.yml`, `check-forbidden-strings.yml`, `check-kilo-generated-artifacts.yml`, `codeql.yml`. All others (jetbrains, docs, nix, publish, containers, watch-opencode-releases, auto-close, disabled/) removed. The allowlist in `script/check-workflows.ts` matches exactly (10 workflows).
...
### Sync 2026-09-19 → upstream v7.7.5

- Upstream integrated from **v7.7.4** to **v7.7.5** (`origin/main` @ release tag `v7.7.5`, commit `01ef456fe7`) in a single merge (`d026dccc95`, "chore: sync upstream v7.7.5 into leocode (offline fork)"); commits after the tag on `origin/main` remain intentionally excluded. No new dependencies added; no online surface reactivated — delta is Agent Manager UX (NewWorktreeDialog mentions `@model`/`@past-chats`/`@worktrees`, worktree-independent slash commands, permission approval recovery, memory-leak fix) plus CLI fixes (bash read-only commands, agent-switch reminders).
- Invariants I1–I10 re-verified green via `bun run check:offline`; typecheck/lint/CI guards/unit suite green (step 3). Offline smoke of the spawned backend: `GET /provider` returns only local/configured providers (privatemode-ai, atomic-chat, lmstudio, anaconda-desktop, llamacpp, ninfer4090) with endpoints all on localhost — no `kilo` provider, no gateway, no remote fetch; `KILO_DISABLE_MODELS_FETCH=true` present in the child env. Visual-regression baselines regenerated once at close-out: existing PNGs unchanged, new `permission-dock-dropdown` story set (12 PNGs) captured by the merge's stories. Packaging gate `bun run compile` green.
