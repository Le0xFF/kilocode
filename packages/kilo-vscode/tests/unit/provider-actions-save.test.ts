...
describe("fetchProviderData", () => {
  ...
  // auth-context tests below no longer apply. The remaining cases verify the
  // still-present offline behaviors (auth-state derivation, key stripping).
  it.each([false, true])("reports auth state for stored-key providers (failure: %s)", async (fail) => {
    const client = {
      provider: {
        list: async () => ({
          data: {
            all: [
              { id: "external", name: "External", source: "custom", env: [], models: {}, key: fail ? "" : "sk-test" },
            ],
            connected: ["external"],
    ...
    expect(result.authStates).toEqual(fail ? {} : { external: "api" })
  })
