...
export function resolveManagedServerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  return {
    ...
    // Offline: never fetch the models.dev catalog. The extension relies on
    // locally-declared OpenAI-compatible providers with no network access.
...
export function buildProxyEnv(): Record<string, string> {
  ...
  const proxyActive = (proxy !== undefined && proxy.trim() !== "") || (proxySet && proxy === undefined)
  if (proxyActive) {
    const noProxyList = Array.isArray(noProxy) ? [...noProxy] : []
    for (const entry of ["127.0.0.1", "localhost"]) {
      if (!noProxyList.includes(entry)) noProxyList.push(entry)
    }
    env.NO_PROXY = noProxyList.join(",")
    env.no_proxy = noProxyList.join(",")
  } else if (Array.isArray(noProxy) && noProxy.length > 0) {
