...
ToolRegistry.register({
  ...
  render(props) {
    ...
    // Deleted files start collapsed above. A chat search match could be inside
    // one. `forceOpenFile` (from MessageList's per-chunk file attribution) names
    // exactly which file's accordion to open. This is tracked separately from the
    // user's own manual toggles: replacing it on every navigation (rather than
    // appending to `expanded`, which never shrinks) closes the previously
    // force-opened file again, so its Pierre diff instance unmounts instead of
    // accumulating one per visited match.
