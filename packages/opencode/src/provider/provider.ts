...
import { inLocalSurface } from "@/kilocode/local-providers" // kilocode_change - cut catalog to local whitelist
import {
...
import { providerKey } from "@/kilocode/provider/cloud-auth" // kilocode_change - cloud-auth helpers served only the removed online loaders
// kilocode_change end
...
export const EMPTY_PUBLIC_INFO: Info = {
  id: ProviderV2.ID.make(""), // kilocode_change - branded empty ID for the offline-catalog no-crash fallback
  source: "custom",
  name: "",
  env: [],
  options: {},
...
export function toPublicInfo(provider: Info | undefined): Info {
  if (!provider) return EMPTY_PUBLIC_INFO // kilocode_change - openai Codex auth plugin: provider is outside the offline catalog (no crash on undefined)
  return JSON.parse(
    JSON.stringify(
      {
        ...provider,
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        // kilocode_change start - cut the catalog to the local whitelist so built-in online providers (incl. env-key'd ones and the apertis/anaconda-desktop overlay) never enter catalog/database/connected; custom/configured providers stay via config.provider
        ...
        function mergeProvider(providerID: ProviderV2.ID, provider: Partial<Info>) {
          ...
          if (!match) {
            // kilocode_change - offline surface: a plugin-auth provider (e.g. openai/Codex OAuth) may
            // be absent from the whitelisted database; still seed it so config/plugin options apply
            providers[providerID] = { ...EMPTY_PUBLIC_INFO, id: providerID, source: "custom" as const, name: "", env: [], models: {} }
            // @ts-expect-error
            providers[providerID] = mergeDeep(providers[providerID], provider)
            return
          }
          // @ts-expect-error
        ...
        for (const [providerID, provider] of configProviders) {
          ...
          for (const [modelID, model] of Object.entries(provider.models ?? {})) {
            ...
            const apiNpm =
              ...
              modelsDev[providerID]?.npm ?? // kilocode_change - custom provider npm from unfiltered catalog
            ...
            const parsedModel: Model = {
              ...
              api: {
                id: apiID,
                npm: apiNpm,
                url: model.provider?.api ?? provider?.api ?? existingModel?.api.url ?? modelsDev[providerID]?.api ?? "", // kilocode_change - custom provider api from unfiltered catalog
              },
